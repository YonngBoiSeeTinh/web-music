...
const PlayListSong = () => {  
    ...
    const setPlaySong = (songIndex) => {
        if (audioRef.current) {
            setCurrentSongIndex(songIndex); // Cập nhật bài hát hiện tại
            setIsPlaying(false); // Cập nhật trạng thái là chưa phát nhạc để kích hoạt useEffect phát nhạc
        }
    };
            ...
            <div className="left">
                ...
                <div className='song-playlist_list'>
                    {arrSong.map((song,index)=>(
                        <PlayListSongItem song={song} key={index} setPlaySong={setPlaySong}/>
                    ))}
